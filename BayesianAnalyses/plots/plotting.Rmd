...
## Import the models
```{r}
model.list <- list(ls = readRDS("model.rds"),
...
plot.noMod.list <- lapply(names(model.list), function(var){
  predicted.values %>% filter(variable %in% var & moderator %in% "noMod") %>%
...
names(plot.noMod.list) <- names(model.list)
